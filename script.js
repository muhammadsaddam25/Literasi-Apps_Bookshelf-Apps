...
const makeBook = (title, author, year, isCompleted) => {
  ...
  if(isCompleted) {
    image.setAttribute('src', 'assets/img/bookread.jfif')
  } else {
    image.setAttribute('src', 'assets/img/bookunread.jpg')
  }
...
const createEditButton = () => {
    return createButton("edit-btn", function(event){
      removeBookFromCompleted(event.target.parentElement);
    })
}
...
bookTitle.addEventListener("keyup", pencarianBuku);

function pencarianBuku(e){
  const bookTitle = e.target.value.toLowerCase();
  ...
  itemTitle.forEach((item) =>{
    ...
    if(isiItem.indexOf(bookTitle) != -1 ){
